...
class ConditionTest extends AbstractTest
{
    ...
    {
        $condition1 = $this->query->getCondition("name LIKE '%lorem%'")->and_($this->query->getCondition('date > NOW()'));
        $condition2 = $this->query->getCondition("name LIKE '%ipsum%'")->and_($this->query->getCondition('date < NOW()'));
    ...
    {
        $condition2 = $this->query->getCondition('date > NOW()')->or_($this->query->getCondition("date < '24.10.2008'"));
        $condition = $this->query->getCondition("name LIKE '%lorem%'")->and_($condition2);
